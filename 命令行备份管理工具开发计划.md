...
| :--------- | :------- | :------- | :------- | :----------------- |
| 版本ID     | Integer  | 是       | 否       | 唯一标识备份版本   |
| 任务ID     | Integer  | 否       | 是       | 对应备份任务的ID   |
| 时间戳     | DateTime | 否       | 否       | 备份完成的时间     |
| 任务名     | String   | 否       | 否       | 备份任务的名称     |
| 备份状态   | Boolean  | 否       | 否       | 备份操作的成功与否 |
| 备份文件名 | String   | 否       | 否       | 生成的备份文件名   |
| 备份大小   | String   | 否       | 否       | 备份文件的大小     |
| 备份路径   | String   | 否       | 否       | 存放备份文件的路径 |
| 版本hash   | String   | 否       | 否       | 备份文件的哈希值   |

- **说明**：该表用于存储每次备份操作的详细记录，通过版本ID唯一标识每个备份版本，任务ID作为外键关联到备份任务表，其他字段分别记录备份操作的关键信息，便于用户查询和管理备份记录。

## 记录备份任务表

| 字段名称 | 数据类型 | 是否主键 | 是否外键 | 描述                   |
| :------- | :------- | :------- | :------- | :--------------------- |
| 任务ID   | Integer  | 是       | 否       | 唯一标识备份任务       |
| 任务名   | String   | 否       | 否       | 备份任务的名称         |
| 目标目录 | String   | 否       | 否       | 需要备份的数据所在路径 |
| 备份目录 | String   | 否       | 否       | 备份文件存放路径       |
| 保留数量 | Integer  | 否       | 否       | 保留的备份版本数量     |

- **说明**：该表用于存储备份任务的基本信息，通过任务ID唯一标识每个备份任务，其他字段分别记录任务的名称、目标目录、备份目录和保留的备份版本数量等关键信息，为备份操作提供配置依据。
