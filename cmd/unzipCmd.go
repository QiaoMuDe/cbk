...
func unzipCmdMain() error {
	...
	unzipFilePath := unzipFile.Get()
	if err := tools.SanitizePath(&unzipFilePath); err != nil {
		return fmt.Errorf("获取ZIP文件绝对路径失败: %w", err)
	}

	// 对指定的输出目录进行清理和获取绝对路径
	unzipOutputDir := unzipOutputDir.Get()
	if err := tools.SanitizePath(&unzipOutputDir); err != nil {
		return fmt.Errorf("获取输出目录绝对路径失败: %w", err)
	}

	// 检查ZIP文件路径是否以.zip结尾
	if !strings.HasSuffix(unzipFilePath, ".zip") {
		return fmt.Errorf("ZIP文件路径必须以.zip结尾: %s", unzipFilePath)
	}

	// 检查指定的ZIP文件路径是否存在
	if _, err := tools.CheckPath(unzipFilePath); err != nil {
		return fmt.Errorf("指定的ZIP文件路径不存在: %s", unzipFilePath)
	}
	...
	// 以点号分隔文件名，获取文件名
	unzipDirName := strings.Split(filepath.Base(unzipFilePath), ".")[0]
	...
	// 解压ZIP文件
	if err := tools.Unzip(unzipFilePath, unzipOutputDir); err != nil {
