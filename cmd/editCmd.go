...
func editCmdMain(db *sqlx.DB) error {
	...
	if editIDS.Get() != "" {
		// 解析多个任务ID
		for _, idStr := range strings.Split(editIDS.Get(), ",") {
			// 检查解析的任务ID是否为空
			if idStr == "" {
...
func editTask(db *sqlx.DB, ids []int) error {
	...
	for _, id := range ids {
		// 检查所有的参数是否都没指定
		if editName.Get() == "" && editRetentionCount.Get() == -1 && editRetentionDays.Get() == -1 && editNoCompression.Get() == -1 && editNewDirName.Get() == "" && editExcludeRules.Get() == "" {
		...
		// 如果指定了-n参数, 则更新任务名
		if editName.Get() != "" {
			task.TaskName = editName.Get()
		}

		// 如果指定了-c参数, 则更新保留数量
		if editRetentionCount.Get() != -1 {
			task.RetentionCount = editRetentionCount.Get()
		}

		// 如果指定了-d参数, 则更新保留天数
		if editRetentionDays.Get() != -1 { // 保留天数默认为0
			task.RetentionDays = editRetentionDays.Get()
		}

		// 如果指定了-nc参数, 则更新是否禁用压缩
		if editNoCompression.Get() != -1 {
			// 检查如果不是true或false则报错
			if editNoCompression.Get() != 1 && editNoCompression.Get() != 0 {
				CL.PrintErrf("-nc 参数不合法, 只能是 0(启用压缩) 或 1(禁用压缩)\n")
				continue
			}

			// 根据参数值更新NoCompression字段
			task.NoCompression = editNoCompression.Get()
		}

		// 如果指定了-bn参数, 则更新备份目录
		var oldDirName, rootPath, newDirName string
		if editNewDirName.Get() != "" {
			newDirName = editNewDirName.Get()
		...
		// 如果指定了-ex参数, 则更新排除规则
		if editExcludeRules.Get() != "" {
			task.ExcludeRules = editExcludeRules.Get()
		}
